...
namespace cjson {
...
#ifdef __DEBUG__
			printf("found recursive declaration %d %d %s\n",iHasRecursive, (int) iRecursive.size(), aStrName.c_str());
#endif
		...
		for (int i=0; i<aMemberList.size(); i++) {
			...
			if (iMethod==NOTMETHOD) {
				...
				if (iMethod==METHOD) {
					...
					apField->getABIInst()._methods.push_back(aMethodField->getABIInst());

					//delete aField; // TODO check if we can delete aField
					aField = 0;			
	...
	field::field* dictionary::decodeMember(DOMDocument* iDocument, const std::string& iID, unsigned long& ioOffset, std::string& ioMemberName, int& iMethod, bool& ioVirtual, 
		...
		if (aElement) {
			...
			if (aType.compare("Field")==0) {
				...
				iMethod = METHOD;
				aField = decodeMethod(iDocument, aElement, ioMemberName, ioVirtual, iRecursive, iHasRecursive, aType.compare("OperatorMethod")==0);
	...
	field::field* dictionary::decodeMethod(DOMDocument* iDocument, DOMElement* iField , std::string& ioMethodName, bool& ioVirtual,
									std::vector<std::string>& iRecursive, int& iHasRecursive, bool iOperator) {
		...
		std::string aVirtual; //type 
		str(aVirtual, iField->getAttribute(str("virtual").get()));
		std::string aReturn; //type 
		str(aReturn, iField->getAttribute(str("returns").get()));
		...
		aField->setOperator(iOperator);

		// Here decoding return value
#ifdef __DEBUG__
		printf("returns: %s\n",aReturn.c_str());
#endif
		field::field* aRField = 0;
		if (aReturn!="") {
			aRField = decodeType(iDocument, aReturn,  iRecursive, iHasRecursive);
			if (aRField) aField->addReturn(aRField->getName());
		}		

		//delete aRField;

		// Here decoding arguments;
		...
		for (int i=0;i<aList->getLength(); i++) {
			...
			str(aType, aArgElement->getAttribute(str("type").get()));
			...
			if (aType!="") {
				...
				aRField = decodeType(iDocument, aType,  iRecursive, iHasRecursive);
#ifdef __DEBUG__
				printf("type: %s \n", aType.c_str() );			
				printf("args: %s %s\n",aRField->getName().c_str() , aType.c_str());
#endif
				aField->addArgument(/*aType*/ aRField->getName()); // this will add args to the ABI structure
	...
	field::field* dictionary::decodeType(DOMDocument* iDocument, const std::string& iID, 
		...
		if (aElement) {
			...
			} else if ((aType.compare("ReferenceType")==0) || (aType.compare("CvQualifiedType")==0)) {
				return decodeReferenceType(iDocument, aElement,  iRecursive, iHasRecursive);
			}
#ifdef __DEBUG__
			else {
	...
	field::field* dictionary::decodeReferenceType(DOMDocument* iDocument, DOMElement* iField,
										std::vector<std::string>& iRecursive, int& iHasRecursive){
		std::string aType; //name
		str(aType, iField->getAttribute(str("type").get()));
