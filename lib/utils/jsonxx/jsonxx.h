...
class jsonxx_exception : public std::exception
{
  ...
  public:
	  ...
	  jsonxx_exception(const std::string& iWhat){
		_what = iWhat;
	  }

	  ~jsonxx_exception() throw() {
	  }

	  virtual const char* what() const throw()
	  {
	    return ("JSONXX Exception:"+_what).c_str();
	  }
...
class Object {
 public:
  ...
  // this complete a vector of values against keys from value map
  const void getKeyValues(std::vector<std::pair<Value,Value> >& ioKeyValues) const;
...
class Value {
 public:
  ...
  template<typename T>
  void import( const T &other ) {
    reset();
   
    // debug
    //std::cout << "[WARN] No support for " << typeid(other).name() << std::endl;
    if (this != &other)
    switch (other.type_) {
      case NULL_:
        import( Null() );
...
/*
void Value::import( const Value &v ) {
    reset();
    import( v );
}
*/
