...
void jsonxx::assertion( const char *file, int line, const char *expression, bool result ) {
    if( !result ) {
	   char buffer[2048];
	   snprintf(buffer,2048, "[JSONXX] expression '%s' failed at %s:%d  ", expression, file, line);
	   throw jsonxx::jsonxx_exception(std::string(buffer));
    }
}
...
bool parse_float(std::istream& input, float& value);
...
const void Object::getKeyValues(std::vector<std::pair<Value, Value> >& ioKeys) const {
	for(container::const_iterator it = value_map_.begin(); it != value_map_.end(); ++it) {
		  Value aKey;
		  aKey.parse("\""+it->first+"\"");
		  ioKeys.push_back(std::pair<Value,Value>(aKey, *(it->second)));
	}
}
...
bool Value::parse(std::istream& input, Value& value) {
    ...
    if (parse_float(input, value.float_value_)) {
        value.type_ = FLOAT_;
        return true;
    }	
...
std::string Value::json() const {
    ...
    jsonxx::Value v;
    v = const_cast<jsonxx::Value*>(this);
...
Value::Value(Value &other) : type_(INVALID_) {
  import( other );
}
