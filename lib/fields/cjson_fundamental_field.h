...
namespace field {
	...
	class FundamentalFactory {
		...
		public :		
			...
			static FundamentalFactory& getRegularInstance();
	...
	class FundamentalFieldFactory : GenericFundamentalFieldFactory {
		...
		public :	
			...
			FundamentalFieldFactory(){
				char buffer[2048];
				demangle(buffer, 2048, typeid(T).name());
				FundamentalFactory::getRegularInstance().addFactory( std::string(buffer) , this);
			}

			FundamentalFieldFactory(const std::string& iName){
				FundamentalFactory::getRegularInstance().addFactory( iName , this);
			}
	...
	#define CREATE_FUNDAMENTAL_TYPE_FACTORY(TYPE,NAME) static cjson::field::FundamentalFieldFactory<TYPE>* NAME##_instance = new cjson::field::FundamentalFieldFactory<TYPE>(); 
	#define CREATE_NAMED_FUNDAMENTAL_TYPE_FACTORY(TYPE,TNAME,NAME) static cjson::field::FundamentalFieldFactory<TYPE>* NAME##_instance = new cjson::field::FundamentalFieldFactory<TYPE>(TNAME); 
