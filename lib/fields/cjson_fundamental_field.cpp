...
namespace field {
		...
		FundamentalFactory& FundamentalFactory::getRegularInstance(){
			static FundamentalFactory* _instance;
			if (!_instance) {
				_instance = new FundamentalFactory();
			}
			return *_instance;
		}
		...
		FundamentalFactory& FundamentalFactory::getInstance(){
			...
			CREATE_NAMED_FUNDAMENTAL_TYPE_FACTORY(short int,"short int",SHORTINT)
			...
			CREATE_NAMED_FUNDAMENTAL_TYPE_FACTORY(wchar_t,"wchar_t",WCHAR_T)
...
//#endif
			if (_factories.find(iFieldType)!=_factories.end()) {
				printf("Registering twice fundamental factory: %s for [%p] from [%p]\n",iFieldType.c_str(),this,iFactory);
				return;
			}
