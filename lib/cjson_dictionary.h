...
namespace cjson {
	...
	class dictionary {
		...
		public :
			...
			std::map<std::string, field::field*>& getResources(){
				return _resources;
			}
		...
		private :
			...
			field::field* decodeMethod(DOMDocument* iDocument, DOMElement* iField , std::string& ioMethodName, bool& ioVirtual, std::vector<std::string>& iRecursive, int& iHasRecursive, bool iOperator);
			...
			//ReferenceType
			field::field* decodeReferenceType(DOMDocument* iDocument, DOMElement* iField,
										std::vector<std::string>& iRecursive, int& iHasRecursive);
